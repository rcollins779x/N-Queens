...
public abstract class Board {
	...
	public static void draw(){
		...
		for(int i = 0; i < dimension; i++){ //looping through the board and printing symbols
			...
			for(Square j: board[i]) System.out.print("|" + j.getSymbol() + "|");
			System.out.print(" " + (dimension - i) + " "); //number on right side

			System.out.print("\n   ");//to get next line

			for(int j = 0; j < dimension; j++) System.out.print(" --- ");
			System.out.print("\n");
		}
		System.out.print("   ");
	...
	public static void solutionFinder() {
		...
		int max = 1, count = 0;
		for(int i = 0; i < dimension; i++) max *= dimension;												//determining max bound

		for(int i = 0; i < max; i++) {
			String temp = String.valueOf(Integer.parseInt(Integer.toString(i, dimension), 10));		//base conversion from 10 to dimension

			while (temp.length() < dimension) temp = "0" + temp;											//pad length with "0"s

			for (int j = 0; j < dimension; j++) pos[j] = Integer.parseInt(String.valueOf(temp.charAt(j)));	//convert char[] to int[]

			if(checkValid(pos)) {																			//if valid print valid pos
				System.out.println(Arrays.toString(pos) + " solution #" + count);
	...
	private static boolean checkValid(int[] iter) {
		...
		for (int i = 0; i < dimension; i++) {
			//Removes vertical duplicates
			for(int j = i + 1; j < dimension; j++)
				if (iter[i] == iter[j])
					return false;
			//if i + index distance from i (\) then return false;
			for(int j = i + 1; j < dimension; j++)
				if (iter[i] == iter[j] - j + i)
					return false;
			//if i - index distance from i (/) then return false;
			for(int j = i + 1; j < dimension; j++)
				if (iter[i] == iter[j] - i + j)
					return false;
		}
		return true;
	}
